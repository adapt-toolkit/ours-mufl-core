#!/usr/bin/env bash
# Fast golden-wire corpus gate: compile the test actor against THIS core, boot a
# local broker, run ONLY tests/corpus.mjs (single packet, no peer traffic).
...
MUFL_STDLIB_PATH="$STDLIB" "$COMPILE" -mp "$AT/meta" -mp "$AT/transactions" corpus_actor.mu 2>&1 \
...
BROKER_URL="ws://127.0.0.1:$PORT" node corpus.mjs 2> >(grep -E 'DRIVER ERR' >&2)
RC=$?
