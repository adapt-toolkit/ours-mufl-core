...
# local broker, run ONLY tests/migapp.mjs (single packet, no peer traffic).
...
cp "$HERE/migapp_actor.mu" "$HERE/migapp.mjs" "$HERE/protocol_container.mm" "$BUILD/"
...
MUFL_STDLIB_PATH="$STDLIB" "$COMPILE" -mp "$AT/meta" -mp "$AT/transactions" migapp_actor.mu 2>&1 \
...
BROKER_URL="ws://127.0.0.1:$PORT" node migapp.mjs 2> >(grep -E 'DRIVER ERR' >&2)
RC=$?
