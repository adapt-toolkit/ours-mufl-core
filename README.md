...
| `a2a_messaging.mm` | `a2a_messaging` | Shared contact + messaging transactions (`generate_invite`, `add_contact`, `send_message`, `send_file`, `remove_contact`, `list_contacts`, inbound `accept_contact`/`receive_message`/`receive_file`) and the state they own; also the control-plane monitoring/config gate (`hidden` so apps cannot override it) and the `core.connect` introduction flow. |
